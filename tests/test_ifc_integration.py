...
def test_parser_reads_physical_instances_and_project_units(tmp_path):
    ...
    _quantity_relation(model,beam,[model.create_entity("IfcQuantityLength",Name="Length",LengthValue=1000),model.create_entity("IfcQuantityVolume",Name="NetVolume",VolumeValue=0.02),model.create_entity("IfcQuantityWeight",Name="Weight",WeightValue=160),model.create_entity("IfcQuantityWeight",Name="NetWeight",WeightValue=157)])
    ...
    assert parsed.profiles[0].gross_weight_kg==160
    assert parsed.profiles[0].net_weight_kg==157
    assert parsed.profiles[0].mass_kg(7850)==160
    assert parsed.profiles[0].net_mass_kg(7850)==157
    assert parsed.profiles[0].mass_source=="IFC Weight"
